...
void calc_threads_blocks (int len, int *num_blocks, int *threads_per_block)
{
...
int f32_sigmoid (float *deviceA, float *deviceB, int lena, void *hipStream)
{
  ...
  calc_threads_blocks(lena, &num_blocks, &threads_per_block);
  hipLaunchKernelGGL(kernel_f32_sigmoid,
                     dim3(num_blocks),
                     dim3(threads_per_block),
                     0, (hipStream_t)hipStream, deviceA, deviceB, lena, 0);
  return 0;
}
...
int f32_grad_sigmoid (float *deviceA, float *deviceB, int lena, void *hipStream)
{
  int num_blocks;
  int threads_per_block;
  calc_threads_blocks(lena, &num_blocks, &threads_per_block);
